...
- SC-022 创建命令（FR-040/041）：命令面板出现「新建任务」「新建项目」，任意非任务文件（如普通笔记）下不灰、可直接执行；默认热键 Mod+Shift+N / Mod+Shift+J（2026-08-24 修订前为 P，与核心 quick switcher 冲突）注册成功且热键面板可改绑，与六条标注命令（L/D/C/K/S/A）无字母冲突；新建任务表单（标题/项目下拉/优先级/到期NL+预览）建档后侧边栏出现该任务、文件内容与捕获框建档字节一致；项目下拉名与 03 Tasks 磁盘文件夹 1:1；到期留空=今天22:00、解析失败拒提交；新建项目落盘三件套=主笔记（type:project）+同名 Dashboard（type:dashboard，两段 dataview 均按 project="<名>" 过滤）+根 Dashboard「独立项目」行追加 `· [[<名> Dashboard]]`；项目名含 / " [ ] 或换行被拒（Notice）；同名笔记已存在时拒绝并 Notice；dashboard_file 配置留空时只建笔记不登记；npm test 含 CREATE_COMMAND_ROWS 与模板/登记/config 纯函数测试
