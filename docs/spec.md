...
- SC-022 创建命令（FR-040/041）：命令面板出现「新建任务」「新建项目」，任意非任务文件（如普通笔记）下不灰、可直接执行；默认热键 Mod+Shift+N / Mod+Shift+P 注册成功且热键面板可改绑，与六条标注命令（L/D/C/K/S/A）无字母冲突；新建任务弹窗用捕获语法建档后侧边栏出现该任务、文件内容与捕获框建档字节一致；新建项目落盘三件套=主笔记（type:project）+同名 Dashboard（type:dashboard，两段 dataview 均按 project="<名>" 过滤）+根 Dashboard「独立项目」行追加 `· [[<名> Dashboard]]`；项目名含 / " [ ] 或换行被拒（Notice）；同名笔记已存在时拒绝并 Notice；dashboard_file 配置留空时只建笔记不登记；npm test 含 CREATE_COMMAND_ROWS 与模板/登记/config 纯函数测试
